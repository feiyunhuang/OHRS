...
public class list extends JPanel{
	...
	private void Labels(){
		...
		for(int i=0;i<OdL.length;i++){
			...
			OdL[i].setText(OdInfor.get(0).get(0));
		}
	}
	...
	private void Buttons(){
		...
		for(int i=0;i<OdL.length;i++){
			OdB[i] = new JButton();
			OdB[i].setSize(button_width-100, button_height);
			OdB[i].setText("查看");
		}
	}
	...
	private void Panels(){
		...
		for(int i=0;i<OdL.length;i++){
			OdP[i] = new JPanel();
			OdP[i].setSize(frame_width-150, panel_height);
			OdP[i].add(OdL[i]);
			OdP[i].add(OdB[i]);
			OdL[i].setLocation(10, 10);
			OdB[i].setLocation(300, 10);
		}
		mainP = new JPanel();
		mainP.setSize(frame_width, frame_height);
